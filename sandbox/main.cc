...
ABSL_FLAG(long long, file_size_limit, NIL_INTEGER, "The limit of file size the executable code can create (especially stdout)");
ABSL_FLAG(std::string, sandbox_dir, NIL_STRING, "The path for sandbox's working directory");
ABSL_FLAG(std::string, src_root_fs_dir, NIL_STRING, "The path for sandbox's root file system is stored (e.g /mnt/gccrootfs)");
